...
import MetricWorker = require('../../workers/MetricWorker');
...
describe('MetricWorker', function () {
    it("should emit a 'duration' event when any emit is intercepted", function (done) {
        ...
        new Service(srvName)
            ...
            .info<IServiceReady>('ready', function (iw) {
                ...
                var duration1 = false;
                var duration2 = false;
                iw.service.info<IMetricIncrement>('iw-metric.increment', function (inc) {
                    if (inc.emit.name === 'test') {
                        increment = true;
                        expect(inc.metricWorkerName).to.be.equal('iw-metric');
                    }
                });
                iw.service.info<IMetricDuration>('iw-metric.duration', function (dur) {
                ...
                iw.service.info<IMetricDuration>('iw-metric-2.duration', function (dur) {
                    if (dur.emit.name === 'test') {
                        duration2 = true;
                        assert.isNumber(dur.duration,"dur duration not a number");
                        expect(dur.duration).to.be.below(2000);
    ...
    it("should not intercept events contained in the 'ignore' option", function (done) {
        ...
        s.info<IMetricIncrement>('iw-metric.increment', function (metricInc) {
        ...
        s.info<IMetricDuration>('iw-metric.duration', function (metricDur) {
        ...
        s.info<IServiceReady>('ready', function () {
            s.tell('test');
            setTimeout(done, 50);
        });
        s.start();
    ...
    it("should intercept an event emitted to another for another service", function (done) {
        ...
        s.info<IMetricIncrement>('iw-metric.increment', function (inc) {
